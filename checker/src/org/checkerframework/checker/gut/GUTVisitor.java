package org.checkerframework.checker.gut;
...
public class GUTVisitor extends BaseTypeVisitor<GUTAnnotatedTypeFactory> {
    ...
    /**
     * The supermethod is currently OK, it only checks for a subtype relationship between the erased
     * types. Depends on what we use as default modifier. Super meth is ok for Any default, but not
     * for peer. TODO Is this really correct?
     */
    @Override
    public boolean isValidUse(
            AnnotatedDeclaredType elemType, AnnotatedDeclaredType use, Tree tree) {
        ...
        if (checkOaM) {
            ...
            if (recvTree != null) {
                ...
                if (recvType.hasEffectiveAnnotation(atypeFactory.LOST)
                    ...
                    List<? extends AnnotationMirror> anns = exelem.getAnnotationMirrors();
    ...
    private boolean isPure(DeclaredType anno) {
        ...
        return anno.toString().equals(org.checkerframework.checker.gut.qual.Pure.class.getName())
    ...
    /**
     * This type validator ensures correct usage of ownership modifiers. It must be run before
     * implicits/defaults, because it should only validate explicitly written qualifiers. TODO The
     * above statement is incorrect. Defaults are already applied.
     */
    private final class GUTTypeValidator extends BaseTypeValidator {

        public GUTTypeValidator(
        ...
        /**
         * Ensure that only one ownership modifier is used, that ownership modifiers are correctly
         * used in static contexts, and check for explicit use of lost.
         */
        @Override
        ...
        protected Void visitParameterizedType(
                AnnotatedDeclaredType type, ParameterizedTypeTree tree) {
            ...
            for (AnnotatedTypeParameterBounds atpb : typevars) {
                if ((atpb.getUpperBound().getKind() != TypeKind.NULL
                                && AnnotatedTypes.containsModifier(
                                        atpb.getUpperBound(), GUTVisitor.this.atypeFactory.VPLOST))
                        || (atpb.getLowerBound().getKind() != TypeKind.NULL
                                && AnnotatedTypes.containsModifier(
                                        atpb.getLowerBound(),
                                        GUTVisitor.this.atypeFactory.VPLOST))) {
                    checker.report(
                            Result.failure(
                                    "uts.vplost.in.bounds", atpb.toString(), type.toString()),
                            tree);
                }
            }
        ...
        public Void visitPrimitive(AnnotatedPrimitiveType type, Tree p) {
            if (type.isAnnotatedInHierarchy(GUTVisitor.this.atypeFactory.ANY)) {
                ...
                if (ann.size() > 1
                        || (ann.size() == 1
                                && !ann.contains(GUTVisitor.this.atypeFactory.BOTTOM))) {
        ...
        /**
         * Each array dimension can only use at most one ownership modifier. The check of the
         * component type is done by the super method.
         */
        @Override
